...
export const GameModals = ({ blankMenu, diacriticMenu, isOpponentLeft, onSelect, onClose }: ModalProps) => {
  ...
  if (blankMenu || diacriticMenu) return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm flex items-center justify-center z-[100] p-4" onClick={onClose}>
      <div className="bg-white p-8 rounded-[3rem] max-w-md w-full shadow-2xl border-t-8 border-indigo-600 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <h3 className="text-2xl font-black text-slate-800 mb-2 text-center">{blankMenu ? "Select Blank Tile" : "Choose Diacritic"}</h3>
        <p className="text-center text-slate-400 text-xs mb-8 uppercase font-bold tracking-widest">{blankMenu ? "Consonants Only" : "Vowels & Tonemarks"}</p>
        <div className="grid grid-cols-5 gap-3 max-h-[45vh] overflow-y-auto pr-2 custom-scrollbar">
          {(blankMenu ? THAI_CONSONANTS : FREE_DIACRITICS).map(char => (
            <button key={char} onClick={() => onSelect(char, !!blankMenu)} className="w-14 h-14 bg-slate-50 border-2 border-slate-100 rounded-2xl text-2xl font-black text-slate-700 hover:bg-indigo-600 hover:text-white hover:scale-110 hover:shadow-lg transition-all">
              {char}
            </button>
          ))}
        </div>
        <button onClick={onClose} className="mt-8 w-full py-4 text-slate-400 font-black hover:text-rose-500 transition-colors uppercase tracking-widest text-xs">Dismiss</button>
      </div>
    </div>
  );
